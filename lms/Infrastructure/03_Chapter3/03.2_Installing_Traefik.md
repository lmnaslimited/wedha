...
```bash
 ...
 ```
## 2\. Create a `docker-compose.ymlFileCreate a D`docker network for Traefik to communicate with your containers:-compose.yml` file to define Traefik and a sample web application. In this example, Traefik will route traffic to a simple web app.
```bash
 docker network create web
 ```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjYyMjAzNzgsNjUxNTAzNjg5LDExOTcwMT
Y0NDNdfQ==
-->